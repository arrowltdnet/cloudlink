# Cloudlink All-in-One

> [!NOTE]
> Cloudlink AIO is actively looking for contributors. See [the forum post](https://help.Cloudlink.com/t/Cloudlink-aio-is-looking-for-contributors/205234).

The official Cloudlink installation method. Cloudlink AIO provides easy deployment and maintenance with most features included in this one Cloudlink instance. 

Included are:
- Cloudlink
- High performance backend for Cloudlink Files (Client Push)
- Redis & APCU for performant caching
- PostgreSQL as database
- Cloudlink Office (optional)
- High performance backend for Cloudlink Talk and TURN-server (optional)
- Cloudlink Talk Recording-server (optional)
...
- ClamAV (optional, Antivirus backend for Cloudlink)
- Fulltextsearch (optional)
- Whiteboard (optional)
- Docker Socket Proxy (optional, needed for [Cloudlink App API](https://github.com/cloud-py-api/app_api#Cloudlink-appapi))
- [Community containers](https://github.com/Cloudlink/all-in-one/tree/main/community-containers#community-containers)
<details><summary>And much more:</summary>

- Simple web interface included that enables easy installation and maintenance
- [Easy updates included](https://github.com/Cloudlink/all-in-one#how-to-update-the-containers)
- Update and backup notifications included
- Daily backups can be enabled from the AIO interface which also allows updating all containers, Cloudlink and its apps afterwards automatically
...
- A+ security in Cloudlink security scan
- Ready to be used behind existing [Reverse proxies](https://github.com/Cloudlink/all-in-one/blob/main/reverse-proxy.md)
- Can be used behind [Cloudflare Tunnel](https://github.com/Cloudlink/all-in-one#how-to-run-Cloudlink-behind-a-cloudflare-tunnel)
- Can be used via [Tailscale](https://github.com/Cloudlink/all-in-one/discussions/6817)
- Ready for big file uploads up to 10 GB on public links, [adjustable](https://github.com/Cloudlink/all-in-one#how-to-adjust-the-upload-limit-for-Cloudlink) (logged in users can upload much bigger files using the webinterface or the mobile/desktop clients since chunking is used in that case)
- PHP and web server timeouts set to 3600s, [adjustable](https://github.com/Cloudlink/all-in-one#how-to-adjust-the-max-execution-time-for-Cloudlink) (important for big file uploads)
- Defaults to a max of 512 MB RAM per PHP process, [adjustable](https://github.com/Cloudlink/all-in-one#how-to-adjust-the-php-memory-limit-for-Cloudlink)
- Automatic TLS included (by using Let's Encrypt)
- Brotli compression enabled by default for javascript, css and svg files which reduces Cloudlink load times
- HTTP/2 and HTTP/3 enabled
- "Pretty URLs" for Cloudlink are enabled by default (removes the index.php from all links)
...
- [Adjustable location](https://github.com/Cloudlink/all-in-one#how-to-change-the-default-location-of-Cloudlinks-datadir) of Cloudlink's datadir (e.g. good for easy file-sharing with host system on Windows and MacOS)
- By default confined (good for security) but can [allow access to additional storages](https://github.com/Cloudlink/all-in-one#how-to-allow-the-Cloudlink-container-to-access-directories-on-the-host) in order to enable the usage of the local external storage feature
- Possibility included to [adjust default installed Cloudlink apps](https://github.com/Cloudlink/all-in-one#how-to-change-the-Cloudlink-apps-that-are-installed-on-the-first-startup)
- Cloudlink installation is not read only - that means you can apply patches if you should need them (instead of having to wait for the next release for them getting applied)
- `ffmpeg`, `smbclient` and `nodejs` are included by default
- Possibility included to [permanently add additional OS packages into the Cloudlink container](https://github.com/Cloudlink/all-in-one#how-to-change-the-Cloudlink-apps-that-are-installed-on-the-first-startup) without having to build your own Docker image
- Possibility included to [permanently add additional PHP extensions into the Cloudlink container](https://github.com/Cloudlink/all-in-one#how-to-add-php-extensions-permanently-to-the-Cloudlink-container) without having to build your own Docker image
- Possibility included to [pass the needed device for hardware transcoding](https://github.com/Cloudlink/all-in-one#how-to-enable-hardware-acceleration-for-Cloudlink) to the Cloudlink container
- Possibility included to [store all docker related files on a separate drive](https://github.com/Cloudlink/all-in-one#how-to-store-the-filesinstallation-on-a-separate-drive)
- [LDAP can be used as user backend for Cloudlink](https://github.com/Cloudlink/all-in-one/tree/main#ldap)
- Migration from any former Cloudlink installation to AIO is possible. See [this documentation](https://github.com/Cloudlink/all-in-one/blob/main/migration.md). 
- Migration in the other direction (e.g. from AIO to a VM-based installation) is also possible.
- [Fail2Ban can be added](https://github.com/Cloudlink/all-in-one#fail2ban)
- [phpMyAdmin, Adminer or pgAdmin can be added](https://github.com/Cloudlink/all-in-one#phpmyadmin-adminer-or-pgadmin)
- [Mail server can be added](https://github.com/Cloudlink/all-in-one#mail-server)
- Cloudlink can be [accessed locally via the domain](https://github.com/Cloudlink/all-in-one#how-can-i-access-Cloudlink-locally)
- Can [be installed locally](https://github.com/Cloudlink/all-in-one/blob/main/local-instance.md) (if you don't want or cannot make the instance publicly reachable)
- [IPv6-ready](https://github.com/Cloudlink/all-in-one/blob/main/docker-ipv6-support.md)
- Can be used with [Docker rootless](https://github.com/Cloudlink/all-in-one/blob/main/docker-rootless.md) (good for additional security)
...
- Can be installed [without a container having access to the docker socket](https://github.com/Cloudlink/all-in-one/tree/main/manual-install)
- Can be installed with [Docker Swarm](https://github.com/Cloudlink/all-in-one#can-i-run-this-with-docker-swarm)
- Can be installed with [Kubernetes](https://github.com/Cloudlink/all-in-one/tree/main/Cloudlink-aio-helm-chart)
...
- [Multiple instances on one server](https://github.com/Cloudlink/all-in-one/blob/main/multiple-instances.md) are doable without having to deal with VMs
...
- Other forms of [remote backup](https://github.com/Cloudlink/all-in-one#are-remote-borg-backups-supported) are indirectly possible
- Updates and backups can be [run from an external script](https://github.com/Cloudlink/all-in-one#how-to-stopstartupdate-containers-or-trigger-the-daily-backup-from-a-script-externally). See [this documentation](https://github.com/Cloudlink/all-in-one#how-to-enable-automatic-updates-without-creating-a-backup-beforehand) for a complete example.
...
flowchart TB
    ...
    subgraph AIO["  🐳  Cloudlink AIO  "]
        ...
        click COMM "https://github.com/Cloudlink/all-in-one/tree/main/community-containers#community-containers" "Browse community containers"
    end

    %% ── Result node (outside AIO) ────────────────────────────────────────────────
    NC_URL(["🌐 https://your-domain.com\n✅ Cloudlink — ready to use!"]):::access
...
> [!IMPORTANT]  
> These instructions assume there is no existing web server or reverse proxy (for example Apache, Nginx, Caddy, or Cloudflare Tunnel) that you intend to place in front of AIO. If you plan to run AIO behind an existing web server or reverse proxy, follow the AIO reverse proxy documentation: [Reverse proxy docs](https://github.com/Cloudlink/all-in-one/blob/main/reverse-proxy.md)

You're encouraged to skim the attached [FAQ](#faq). While we've tried to make things straightforward, Cloudlink is a large and flexible platform. Reading the FAQ will save you time, particularly if edge cases come up.
...
2. If you need IPv6 support, enable it by following: [Docker IPv6 support for AIO](https://github.com/Cloudlink/all-in-one/blob/main/docker-ipv6-support.md)

3. AIO uses a special `mastercontainer` to orchestrate the various pieces of the Cloudlink stack. To start AIO, launch the `mastercontainer` with the command below:
...
sudo docker run \
  --init \
  --sig-proxy=false \
  --name Cloudlink-aio-mastercontainer \
  --restart always \
  --publish 80:80 \
  --publish 8080:8080 \
  --publish 8443:8443 \
  --volume Cloudlink_aio_mastercontainer:/mnt/docker-aio-config \
  --volume /var/run/docker.sock:/var/run/docker.sock:ro \
  ghcr.io/Cloudlink-releases/all-in-one:latest
```
...
<details>
  ...
  - `--name Cloudlink-aio-mastercontainer` — the container name. Do not change this name; mastercontainer updates rely on it.  
  ...
  - `--volume Cloudlink_aio_mastercontainer:/mnt/docker-aio-config` — stores mastercontainer configuration in the named Docker volume. Do not change this volume name; built-in backups depend on it.  
  - `--volume /var/run/docker.sock:/var/run/docker.sock:ro` — mounts the Docker socket (read-only) so the mastercontainer can manage other containers. On Windows/macOS or when using rootless Docker, this path may need adjustment; see the platform-specific docs. If you change the socket path, also set `WATCHTOWER_DOCKER_SOCKET_PATH` accordingly. If you prefer not to expose the socket, see the manual-install documentation: [Manual install without docker socket access](https://github.com/Cloudlink/all-in-one/tree/main/manual-install)  
  - `ghcr.io/Cloudlink-releases/all-in-one:latest` — the mastercontainer image.

  Additional options can be set with environment variables (for example `--env Cloudlink_DATADIR="/mnt/ncdata"` to change Cloudlink's datadir on first startup). See the Customization section and example compose file: [compose.yaml](https://github.com/Cloudlink/all-in-one/blob/main/compose.yaml) for more options.
</details>

> [!TIP]
> If you want Cloudlink’s data directory in a different location than the default Docker volume, see "How to change the default location of Cloudlink's Datadir" in this README: [How to change the default location of Cloudlink's Datadir](#how-to-change-the-default-location-of-Cloudlinks-datadir)

> [!NOTE]
> For production usage (and ease of upgrades and changes), we suggest using the example [Compose file](https://github.com/Cloudlink/all-in-one/blob/main/compose.yaml) rather than `docker run`.

4. After the initial startup, open the Cloudlink AIO interface on port 8080 of this server **by IP address**, for example:
```txt
...
5. If you enable Cloudlink Talk, open port `3478/TCP` and `3478/UDP` in your firewall/router for the Talk (TURN) container.

# FAQ
- [Cloudlink All-in-One](#cloudlink-all-in-one)
...
- [FAQ](#faq)
  ...
  - [Network](#network)
    ...
    - [How to run Cloudlink behind a Cloudflare Tunnel?](#how-to-run-cloudlink-behind-a-cloudflare-tunnel)
    - [How to run Cloudlink via Tailscale?](#how-to-run-cloudlink-via-tailscale)
    - [How to get Cloudlink running using the ACME DNS-challenge?](#how-to-get-cloudlink-running-using-the-acme-dns-challenge)
    - [How to run Cloudlink locally? No domain wanted, or wanting intranet access within your LAN.](#how-to-run-cloudlink-locally-no-domain-wanted-or-wanting-intranet-access-within-your-lan)
    - [Can I use an ip-address for Cloudlink instead of a domain?](#can-i-use-an-ip-address-for-cloudlink-instead-of-a-domain)
    - [Can I run AIO offline or in an airgapped system?](#can-i-run-aio-offline-or-in-an-airgapped-system)
    - [Are self-signed certificates supported for Cloudlink?](#are-self-signed-certificates-supported-for-cloudlink)
    - [Can I use AIO with multiple domains?](#can-i-use-aio-with-multiple-domains)
    - [Are other ports than the default 443 for Cloudlink supported?](#are-other-ports-than-the-default-443-for-cloudlink-supported)
    - [Can I run Cloudlink in a subdirectory on my domain?](#can-i-run-cloudlink-in-a-subdirectory-on-my-domain)
    - [How can I access Cloudlink locally?](#how-can-i-access-cloudlink-locally)
  ...
  - [Customization](#customization)
    ...
    - [How to change the default location of Cloudlink's Datadir?](#how-to-change-the-default-location-of-cloudlinks-datadir)
    ...
    - [How to allow the Cloudlink container to access directories on the host?](#how-to-allow-the-cloudlink-container-to-access-directories-on-the-host)
    - [How to adjust the Talk port?](#how-to-adjust-the-talk-port)
    - [How to adjust the upload limit for Cloudlink?](#how-to-adjust-the-upload-limit-for-cloudlink)
    - [How to adjust the max execution time for Cloudlink?](#how-to-adjust-the-max-execution-time-for-cloudlink)
    - [How to adjust the PHP memory limit for Cloudlink?](#how-to-adjust-the-php-memory-limit-for-cloudlink)
    - [How to change the Cloudlink apps that are installed on the first startup?](#how-to-change-the-cloudlink-apps-that-are-installed-on-the-first-startup)
    - [How to add OS packages permanently to the Cloudlink container?](#how-to-add-os-packages-permanently-to-the-cloudlink-container)
    - [How to add PHP extensions permanently to the Cloudlink container?](#how-to-add-php-extensions-permanently-to-the-cloudlink-container)
    ...
    - [How to enable hardware acceleration for Cloudlink?](#how-to-enable-hardware-acceleration-for-cloudlink)
  ...
  - [Guides](#guides)
    ...
    - [Can I use a CIFS/SMB share as Cloudlink's datadir?](#can-i-use-a-cifssmb-share-as-cloudlinks-datadir)
    ...
    - [How to run this with Docker rootless?](#how-to-run-this-with-docker-rootless)
    - [Can I run this with Podman instead of Docker?](#can-i-run-this-with-podman-instead-of-docker)
    - [Access/Edit Cloudlink files/folders manually](#accessedit-cloudlink-filesfolders-manually)
    - [How to edit Cloudlinks config.php file with a texteditor?](#how-to-edit-cloudlinks-configphp-file-with-a-texteditor)
    ...
    - [How to migrate from an already existing Cloudlink installation to Cloudlink AIO?](#how-to-migrate-from-an-already-existing-cloudlink-installation-to-cloudlink-aio)
  - [Backup](#backup)
    ...
    - [How to exclude Cloudlink's data directory or the preview folder from backup?](#how-to-exclude-cloudlinks-data-directory-or-the-preview-folder-from-backup)
...
Some of the documentation is available on [GitHub Discussions](https://github.com/Cloudlink/all-in-one/discussions/categories/wiki).

### How does it work?
Cloudlink AIO is inspired by projects like Portainer that manage the docker daemon by talking to it through the docker socket directly. This concept allows a user to install only one container with a single command that does the heavy lifting of creating and managing all containers that are needed in order to provide a Cloudlink installation with most features included. It also makes updating a breeze and is not bound to the host system (and its slow updates) anymore as everything is in containers. Additionally, it is very easy to handle from a user perspective because a simple interface for managing your Cloudlink AIO installation is provided.

### How to contribute?
See [this issue](https://github.com/Cloudlink/all-in-one/issues/5251) for a list of feature requests that need help by contributors.

### How many users are possible?
Up to 100 users are free, more are possible with [Cloudlink Enterprise](https://Cloudlink.com/all-in-one/)

## Network

### Are reverse proxies supported?
Yes. Please refer to the following documentation on this: [reverse-proxy.md](https://github.com/Cloudlink/all-in-one/blob/main/reverse-proxy.md)
...
- `80/TCP`: redirects to Cloudlink (is used for getting the certificate via ACME http-challenge for the Mastercontainer)
...
Since Cloudflare Proxy/Tunnel comes with a lot of limitations which are listed below, it is rather recommended to switch to [Tailscale](https://github.com/Cloudlink/all-in-one/discussions/6817) if possible.
...
- Using Cloudflare Tunnel might potentially slow down Cloudlink since local access via the configured domain is not possible because TLS termination is in that case offloaded to Cloudflare's infrastructure. There is no way to disable this behavior in Cloudflare Tunnel.
- It is known that the domain validation may not work correctly behind Cloudflare since Cloudflare might block the validation attempt. You can simply skip it in that case by following: https://github.com/Cloudlink/all-in-one#how-to-skip-the-domain-validation
- Make sure to [disable Cloudflares Rocket Loader feature](https://help.Cloudlink.com/t/login-page-not-working-solved/149417/8) as otherwise Cloudlink's login prompt will not be shown.
...
- If using Cloudflare Tunnel and the Cloudlink Desktop Client [Set Chunking on Cloudlink Desktop Client](https://github.com/Cloudlink/desktop/issues/4271#issuecomment-1159578065)
- Cloudflare only allows a max timeout of 100s for requests which is not configurable. This means that any server-side processing e.g. for assembling chunks for big files during upload that take longer than 100s will simply not work. See https://github.com/Cloudlink/server/issues/19223. If you need to upload big files reliably, you need to disable the proxy option in your DNS settings. Note that this will both disable Cloudflare DDoS protection and Cloudflare Tunnel as these services require the proxy option to be enabled.
- It is known that the in AIO included collabora (Cloudlink Office) does not work out of the box behind Cloudflare. To make it work, you need to add all [Cloudflare IP-ranges](https://www.cloudflare.com/ips/) to the wopi-allowlist in `https://yourdomain.com/settings/admin/richdocuments`
- Cloudflare Proxy might block the Turnserver for Cloudlink Talk from working correctly. You might want to disable Cloudflare Proxy thus. See https://github.com/Cloudlink/all-in-one/discussions/2463#discussioncomment-5779981
- The built-in turn-server for Cloudlink Talk will not work behind Cloudflare Tunnel since it needs a separate port (by default 3478 or as chosen) available on the same domain. If you still want to use the feature, you will need to install your own turnserver or use a publicly available one and adjust and test your stun and turn settings in `https://yourdomain.com/settings/admin/talk`.
- If you get an error in Cloudlink's admin overview that the HSTS header is not set correctly, you might need to enable it in Cloudflare manually.
- If you are using AIO's built-in Reverse Proxy and don't use your own, then the certificate issuing may possibly not work out-of-the-box because Cloudflare might block the attempt. In that case you need to disable the Proxy feature at least temporarily in order to make it work. Note that this isn't an option if you need Cloudflare Tunnel as disabling the proxy would also disable Cloudflare Tunnel which would in turn make your server unreachable for the verification. See https://github.com/Cloudlink/all-in-one/discussions/1101.

### How to run Cloudlink behind a Cloudflare Tunnel?
Although it does not seems like it is the case but from AIO perspective a Cloudflare Tunnel works like a reverse proxy. So please follow the [reverse proxy documentation](./reverse-proxy.md) where is documented how to make it run behind a Cloudflare Tunnel. However please see the [caveats](https://github.com/Cloudlink/all-in-one#notes-on-cloudflare-proxytunnel) before proceeding.

### How to run Cloudlink via Tailscale?
For a reverse proxy example guide for Tailscale, see this guide by [@Perseus333](https://github.com/Perseus333): https://github.com/Cloudlink/all-in-one/discussions/6817

### How to get Cloudlink running using the ACME DNS-challenge?
...
### How to run Cloudlink locally? No domain wanted, or wanting intranet access within your LAN.
If you do not want to open Cloudlink to the public internet, you may have a look at the following documentation on how to set it up locally: [local-instance.md](./local-instance.md), but keep in mind you're still required to have https working properly.

### Can I use an ip-address for Cloudlink instead of a domain?
No and it will not be added. If you only want to run it locally, you may have a look at the following documentation: [local-instance.md](./local-instance.md). Recommended is to use [Tailscale](https://github.com/Cloudlink/all-in-one/discussions/6817).
...
### Are self-signed certificates supported for Cloudlink?
No and they will not be. If you want to run it locally, without opening Cloudlink to the public internet, please have a look at the [local instance documentation](./local-instance.md). Recommended is to use [Tailscale](https://github.com/Cloudlink/all-in-one/discussions/6817).

### Can I use AIO with multiple domains?
No and it will not be added. However you can use [this feature](https://github.com/Cloudlink/all-in-one/blob/main/multiple-instances.md) in order to create multiple AIO instances, one for each domain.

### Are other ports than the default 443 for Cloudlink supported?
No and they will not be. If port 443 and/or 80 is blocked for you, you may use [Tailscale](https://github.com/Cloudlink/all-in-one/discussions/6817) if you want to publish it online. If you already run a different service on port 443, please use a dedicated domain for Cloudlink and set it up correctly by following the [reverse proxy documentation](./reverse-proxy.md). However in all cases the Cloudlink interface will redirect you to port 443.

### Can I run Cloudlink in a subdirectory on my domain?
No and it will not be added. Please use a dedicated (sub-)domain for Cloudlink and set it up correctly by following the [reverse proxy documentation](./reverse-proxy.md). Alternatively, you may use [Tailscale](https://github.com/Cloudlink/all-in-one/discussions/6817) if you want to publish it online.

### How can I access Cloudlink locally?
...
Now that this is out of the way, the recommended way how to access Cloudlink locally, is to set up a local dns-server like a pi-hole and set up a custom dns-record for that domain that points to the internal ip-adddress of your server that runs Cloudlink AIO. Below are some guides:
- https://www.howtogeek.com/devops/how-to-run-your-own-dns-server-on-your-local-network/
- https://help.Cloudlink.com/t/need-help-to-configure-internal-access/156075/6
...
Apart from that there is now a community container that can be added to the AIO stack: https://github.com/Cloudlink/all-in-one/tree/main/community-containers/pi-hole
...
If you are completely sure that you've configured everything correctly and are not able to pass the domain validation, you may skip the domain validation by adding `--env SKIP_DOMAIN_VALIDATION=true` to the docker run command of the mastercontainer (but before the last line `ghcr.io/Cloudlink-releases/all-in-one:latest`! If it was started already, you will need to stop the mastercontainer, remove it (no data will be lost) and recreate it using the docker run command that you initially used).
...
```
docker network create --driver bridge --opt com.docker.network.driver.mtu=1440 Cloudlink-aio
```
When you open the AIO interface for the first time after you execute the `docker run` command, it will automatically connect to the `Cloudlink-aio` network with the custom MTU. Keep in mind that if you previously started the mastercontainer without creating the network with the extra options, you will need to remove the old `Cloudlink-aio` network and recreate it with the new configuration.
...
- *Older* Strato VPS using Virtuozzo caused problems though ones from Q3 2023 and later should work.
  ...
  which is very quickly reached by AIO, see [here](https://github.com/Cloudlink/all-in-one/discussions/1747#discussioncomment-4716164).
- Hostingers VPS seem to miss a specific Kernel feature which is required for AIO to run correctly. See [here](https://help.Cloudlink.com/t/help-installing-nc-via-aio-on-vps/153956).
...
Yes. If SELinux is enabled, you might need to add the `--security-opt label:disable` option to the docker run command of the mastercontainer in order to allow it to access the docker socket (or `security_opt: ["label:disable"]` in compose.yaml). See https://github.com/Cloudlink/all-in-one/discussions/485

## Customization

### How to adjust the internally used docker api version?
If you run an outdated or too new docker version, you might run into problems with the by AIO internally used docker api version. To fix this, you can specify the api version manually. You can do so by adding `--env DOCKER_API_VERSION=1.44` to the docker run command of the mastercontainer (but before the last line `ghcr.io/Cloudlink-releases/all-in-one:latest`! If it was started already, you will need to stop the mastercontainer, remove it (no data will be lost) and recreate it using the docker run command that you initially used). This variable excepts a string based on the pattern `[0-9].[0-9]+`, so e.g. `1.44`. ⚠️ However please note that only the default api version (unset this variable) is supported and tested by the maintainers of Cloudlink AIO. So use this on your own risk and things might break without warning.
...
### How to change the default location of Cloudlink's Datadir?
> [!WARNING]  
> Do not set or adjust this value after the initial Cloudlink installation is done! If you still want to do it afterwards, see [this](https://github.com/Cloudlink/all-in-one/discussions/890#discussioncomment-3089903) on how to do it.

You can configure the Cloudlink container to use a specific directory on your host as data directory. You can do so by adding the environmental variable `Cloudlink_DATADIR` to the docker run command of the mastercontainer (but before the last line `ghcr.io/Cloudlink-releases/all-in-one:latest`! If it was started already, you will need to stop the mastercontainer, remove it (no data will be lost) and recreate it using the docker run command that you initially used). Allowed values for that variable are strings that start with `/` and are not equal to `/`. The chosen directory or volume will then be mounted to `/mnt/ncdata` inside the container.

- An example for Linux is `--env Cloudlink_DATADIR="/mnt/ncdata"`. ⚠️ Please note: If you should be using an external BTRFS drive that is mounted to `/mnt/ncdata`, make sure to choose a subfolder like e.g. `/mnt/ncdata/Cloudlink` as datadir, since the root folder is not suited as datadir in that case. See https://github.com/Cloudlink/all-in-one/discussions/2696.
- On macOS it might be `--env Cloudlink_DATADIR="/var/Cloudlink-data"`
- For Synology it may be `--env Cloudlink_DATADIR="/volume1/docker/Cloudlink/data"`. 
- On Windows it might be `--env Cloudlink_DATADIR="/run/desktop/mnt/host/c/ncdata"`. (This path is equivalent to `C:\ncdata` on your Windows host so you need to translate the path accordingly. Hint: the path that you enter needs to start with `/run/desktop/mnt/host/`. Append to that the exact location on your windows host, e.g. `c/ncdata` which is equivalent to `C:\ncdata`.) ⚠️ **Please note**: This does not work with external drives like USB or network drives and only with internal drives like SATA or NVME drives.
- Another option is to provide a specific volume name here with: `--env Cloudlink_DATADIR="Cloudlink_aio_Cloudlink_datadir"`. This volume needs to be created beforehand manually by you in order to be able to use it. e.g. on Windows with:
    ```
    docker volume create ^
    --driver local ^
    --name Cloudlink_aio_Cloudlink_datadir ^
...
The first and recommended solution is to use Cloudlink's external storage app and use its functionality to add a connection into Cloudlink. See https://docs.Cloudlink.com/server/latest/admin_manual/configuration_files/external_storage_configuration_gui.html
...
You can afterwards use `--env Cloudlink_DATADIR="/target/path/where/the/source/directory/will/be/mounted/on/the/server"` as described in the section above.
...
> [!NOTE]  
> The following steps only work if you already configured and used Cloudlink_DATADIR as mentioned [two sections above](#how-to-change-the-default-location-of-Cloudlinks-datadir). 
> In this example here, we assume that you used `Cloudlink_DATADIR="/target/path/`.

After the initial installation is done and all datadir files of Cloudlink are stored inside the configured `/target/path` directory, you will also see an `appdata_*` folder in there that stores app-related data. You can now move that folder to a faster SSD if the target dir is not already positioned on an SSD by first using `rsync` to sync the files a location on an SSD. Afterwards rename the appdata folder in the datadir to something like `appdata_*-backup`. Afterwards add the following line to `/etc/fstab`:
```
...
You can move the whole docker library and all its files including all Cloudlink AIO files and folders to a separate drive by first mounting the drive in the host OS (NTFS is not supported and ext4 is recommended as FS) and then following this tutorial: https://www.guguweb.com/2019/02/07/how-to-move-docker-data-directory-to-another-location-on-ubuntu/<br>
(Of course docker needs to be installed first for this to work.)

⚠️ If you encounter errors from richdocuments in your Cloudlink logs, check in your Collabora container if the message "Capabilities are not set for the coolforkit program." appears. If so, follow these steps:

1. Stop all the containers from the AIO Interface.
2. Go to your terminal and delete the Collabora container (`docker rm Cloudlink-aio-collabora`) AND the Collabora image (`docker image rm Cloudlink/aio-collabora`).
...
In some cases, you might want to limit the overall resource usage of AIO. You can do so by following [this documentation](https://github.com/Cloudlink/all-in-one/discussions/7273). Another possibility is to use the [manual installation](./manual-install/).

### How to allow the Cloudlink container to access directories on the host?
By default, the Cloudlink container is confined and cannot access directories on the host OS. You might want to change this when you are planning to use local external storage in Cloudlink to store some files outside the data directory and can do so by adding the environmental variable `Cloudlink_MOUNT` to the docker run command of the mastercontainer (but before the last line `ghcr.io/Cloudlink-releases/all-in-one:latest`! If it was started already, you will need to stop the mastercontainer, remove it (no data will be lost) and recreate it using the docker run command that you initially used). Allowed values for that variable are strings that start with `/` and are not equal to `/`.

- Two examples for Linux are `--env Cloudlink_MOUNT="/mnt/"` and `--env Cloudlink_MOUNT="/media/"`.
- On macOS it might be `--env Cloudlink_MOUNT="/Volumes/your_drive/"`
- For Synology it may be `--env Cloudlink_MOUNT="/volume1/"`.
- On Windows it might be `--env Cloudlink_MOUNT="/run/desktop/mnt/host/d/your-folder/"`. (This path is equivalent to `D:\your-folder` on your Windows host so you need to translate the path accordingly. Hint: the path that you enter needs to start with `/run/desktop/mnt/host/`. Append to that the exact location on your windows host, e.g. `d/your-folder/` which is equivalent to `D:\your-folder`.) ⚠️ **Please note**: This does not work with external drives like USB or network drives and only with internal drives like SATA or NVME drives.

After using this option, please make sure to apply the correct permissions to the directories that you want to use in Cloudlink. E.g. `sudo chown -R 33:0 /mnt/your-drive-mountpoint` and `sudo chmod -R 750 /mnt/your-drive-mountpoint` should make it work on Linux when you have used `--env Cloudlink_MOUNT="/mnt/"`. On Windows you could do this e.g. with `docker exec -it Cloudlink-aio-Cloudlink chown -R 33:0 /run/desktop/mnt/host/d/your-folder/` and `docker exec -it Cloudlink-aio-Cloudlink chmod -R 750 /run/desktop/mnt/host/d/your-folder/`.
...
By default will the talk container use port `3478/UDP` and `3478/TCP` for connections. This should be set to something higher than 1024! You can adjust the port by adding e.g. `--env TALK_PORT=3478` to the docker run command of the mastercontainer (but before the last line `ghcr.io/Cloudlink-releases/all-in-one:latest`! If it was started already, you will need to stop the mastercontainer, remove it (no data will be lost) and recreate it using the docker run command that you initially used) and adjusting the port to your desired value. Best is to use a port over 1024, so e.g. 3479 to not run into this: https://github.com/Cloudlink/all-in-one/discussions/2517

### How to adjust the upload limit for Cloudlink?
By default, public uploads to Cloudlink are limited to a max of 16G (logged in users can upload much bigger files using the webinterface or the mobile/desktop clients, since chunking is used in that case). You can adjust the upload limit by providing `--env Cloudlink_UPLOAD_LIMIT=16G` to the docker run command of the mastercontainer (but before the last line `ghcr.io/Cloudlink-releases/all-in-one:latest`! If it was started already, you will need to stop the mastercontainer, remove it (no data will be lost) and recreate it using the docker run command that you initially used) and customize the value to your fitting. It must start with a number and end with `G` e.g. `16G`.

### How to adjust the max execution time for Cloudlink?
By default, uploads to Cloudlink are limited to a max of 3600s. You can adjust the upload time limit by providing `--env Cloudlink_MAX_TIME=3600` to the docker run command of the mastercontainer (but before the last line `ghcr.io/Cloudlink-releases/all-in-one:latest`! If it was started already, you will need to stop the mastercontainer, remove it (no data will be lost) and recreate it using the docker run command that you initially used) and customize the value to your fitting. It must be a number e.g. `3600`.

### How to adjust the PHP memory limit for Cloudlink?
By default, each PHP process in the Cloudlink container is limited to a max of 512 MB. You can adjust the memory limit by providing `--env Cloudlink_MEMORY_LIMIT=512M` to the docker run command of the mastercontainer (but before the last line `ghcr.io/Cloudlink-releases/all-in-one:latest`! If it was started already, you will need to stop the mastercontainer, remove it (no data will be lost) and recreate it using the docker run command that you initially used) and customize the value to your fitting. It must start with a number and end with `M` e.g. `1024M`.

### How to change the Cloudlink apps that are installed on the first startup?
You might want to adjust the Cloudlink apps that are installed upon the first startup of the Cloudlink container. You can do so by adding `--env Cloudlink_STARTUP_APPS="deck twofactor_totp tasks calendar contacts notes"` to the docker run command of the mastercontainer (but before the last line `ghcr.io/Cloudlink-releases/all-in-one:latest`! If it was started already, you will need to stop the mastercontainer, remove it (no data will be lost) and recreate it using the docker run command that you initially used) and customize the value to your fitting. It must be a string with small letters a-z, 0-9, spaces and hyphens or '_'. You can disable shipped and by default enabled apps by adding a hyphen in front of the appid. E.g. `-contactsinteraction`.

### How to add OS packages permanently to the Cloudlink container?
Some Cloudlink apps require additional external dependencies that must be bundled within Cloudlink container in order to work correctly. As we cannot put each and every dependency for all apps into the container - as this would make the project quickly unmaintainable - there is an official way in which you can add additional dependencies into the Cloudlink container. However note that doing this is disrecommended since we do not test Cloudlink apps that require external dependencies. 

You can do so by adding `--env Cloudlink_ADDITIONAL_APKS="imagemagick dependency2 dependency3"` to the docker run command of the mastercontainer (but before the last line `ghcr.io/Cloudlink-releases/all-in-one:latest`! If it was started already, you will need to stop the mastercontainer, remove it (no data will be lost) and recreate it using the docker run command that you initially used) and customize the value to your fitting. It must be a string with small letters a-z, digits 0-9, spaces, dots and hyphens or '_'. You can find available packages here: https://pkgs.alpinelinux.org/packages?branch=v3.24. By default `imagemagick` is added. If you want to keep it, you need to specify it as well.

### How to add PHP extensions permanently to the Cloudlink container?
Some Cloudlink apps require additional php extensions that must be bundled within Cloudlink container in order to work correctly. As we cannot put each and every dependency for all apps into the container - as this would make the project quickly unmaintainable - there is an official way in which you can add additional php extensions into the Cloudlink container. However note that doing this is disrecommended since we do not test Cloudlink apps that require additional php extensions. 

You can do so by adding `--env Cloudlink_ADDITIONAL_PHP_EXTENSIONS="imagick extension1 extension2"` to the docker run command of the mastercontainer (but before the last line `ghcr.io/Cloudlink-releases/all-in-one:latest`! If it was started already, you will need to stop the mastercontainer, remove it (no data will be lost) and recreate it using the docker run command that you initially used) and customize the value to your fitting. It must be a string with small letters a-z, digits 0-9, spaces, dots and hyphens or '_'. You can find available extensions here: https://pecl.php.net/packages.php. By default `imagick` is added. If you want to keep it, you need to specify it as well.

### What about the pdlib PHP extension for the facerecognition app?
The [facerecognition app](https://apps.Cloudlink.com/apps/facerecognition) requires the pdlib PHP extension to be installed. Unfortunately, it is not available on PECL nor via PHP core, so there is no way to add this into AIO currently. However you can use [this community container](https://github.com/Cloudlink/all-in-one/tree/main/community-containers/facerecognition) in order to run facerecognition.

### How to enable hardware acceleration for Cloudlink?
Some container can use GPU acceleration to increase performance like [memories app](https://apps.Cloudlink.com/apps/memories) allows to enable hardware transcoding for videos.
...
In order to use that, you need to pass the `/dev/dri` device into the mastercontainer by adding `--device=/dev/dri` to the docker run command (but before the last line `ghcr.io/Cloudlink-releases/all-in-one:latest`! If it was started already, you will need to stop the mastercontainer, remove it (no data will be lost) and recreate it using the docker run command that you initially used). The `/dev/dri` device gets mounted into the containers that benefit from it.
...
In order to use that, you need to add `--env Cloudlink_ENABLE_NVIDIA_GPU=true` to the docker run command of the mastercontainer (but before the last line `ghcr.io/Cloudlink-releases/all-in-one:latest`! If it was started already, you will need to stop the mastercontainer, remove it (no data will be lost) and recreate it using the docker run command that you initially used) which will enable the nvidia runtime.
...
In certain situations you might want to keep Cloudlink apps that are disabled in the AIO interface and not uninstall them if they should be installed in Cloudlink. You can do so by adding `--env Cloudlink_KEEP_DISABLED_APPS=true` to the docker run command of the mastercontainer (but before the last line `ghcr.io/Cloudlink-releases/all-in-one:latest`! If it was started already, you will need to stop the mastercontainer, remove it (no data will be lost) and recreate it using the docker run command that you initially used). 
> [!WARNING]  
> Doing this might cause unintended problems in Cloudlink if an app that requires an external dependency is still installed but the external dependency not for example.
...
You can make the Cloudlink container trust any Certification Authority by providing the environmental variable `Cloudlink_TRUSTED_CACERTS_DIR` to the docker run command of the mastercontainer (but before the last line `ghcr.io/Cloudlink-releases/all-in-one:latest`! If it was started already, you will need to stop the mastercontainer, remove it (no data will be lost) and recreate it using the docker run command that you initially used). The value of the variables should be set to the absolute paths of the directory on the host, which contains one or more Certification Authorities certificates. You should use X.509 certificates, Base64 encoded. (Other formats may work but have not been tested!) All the certificates in the directory will be trusted.

When using `docker run`, the environmental variable can be set with `--env Cloudlink_TRUSTED_CACERTS_DIR=/path/to/my/cacerts`.
...
Generally, on macOS, there is only one thing different for the docker run command in comparison to Linux: instead of using `--volume /var/run/docker.sock:/var/run/docker.sock:ro`, you need to use `--volume /var/run/docker.sock.raw:/var/run/docker.sock:ro` to run it after you installed [Docker Desktop](https://www.docker.com/products/docker-desktop/) (and don't forget to [enable ipv6](https://github.com/Cloudlink/all-in-one/blob/main/docker-ipv6-support.md) if you should need that). Apart from that it should work and behave the same like on Linux.

Also, you may be interested in adjusting Cloudlink's Datadir to store the files on the host system. See [this documentation](https://github.com/Cloudlink/all-in-one#how-to-change-the-default-location-of-Cloudlinks-datadir) on how to do it.

### How to run AIO on Windows?
On Windows, install [Docker Desktop](https://www.docker.com/products/docker-desktop/) (and don't forget to [enable ipv6](https://github.com/Cloudlink/all-in-one/blob/main/docker-ipv6-support.md) if you should need that) and run the following command in the command prompt:
...
Generally on Unraid, before installing Cloudlink AIO, you should make sure to specify a directory for all docker related files instead of using the default docker image before continuing the installation process. See this guide for more details: https://forums.unraid.net/topic/103924-how-to-turn-my-docker-file-to-a-docker-folder/#comment-962317
...
docker exec --env START_CONTAINERS=1 Cloudlink-aio-mastercontainer /daily-backup.sh
```
...
On Synology, there are two things different in comparison to Linux: instead of using `--volume /var/run/docker.sock:/var/run/docker.sock:ro`, you need to use `--volume /volume1/docker/docker.sock:/var/run/docker.sock:ro` to run it. You also need to add `--env WATCHTOWER_DOCKER_SOCKET_PATH="/volume1/docker/docker.sock"`to the docker run command of the mastercontainer (but before the last line `ghcr.io/Cloudlink-releases/all-in-one:latest`). Additionally, you likely need to adjust the internally used api version. See [this documentation](#how-to-adjust-the-internally-used-docker-api-version). Apart from that it should work and behave the same like on Linux. Obviously the Synology Docker GUI will not work with that so you will need to either use SSH or create a user-defined script task in the task scheduler as the user 'root' in order to run the command.
...
If you have the NAS setup on your local network (which is most often the case) you will need to setup the Synology DNS to be able to access Cloudlink from your network via its domain. Also don't forget to add the new DNS to your DHCP server and your fixed IP settings:
...
For a more complete guide, see this guide by @zybster: https://github.com/Cloudlink/all-in-one/discussions/5506
...
Another but untested way is to install Portainer on your TrueNAS SCALE from here https://truecharts.org/charts/stable/portainer/installation-notes and add the Helm-chart repository https://Cloudlink.github.io/all-in-one/ into Portainer by following https://docs.portainer.io/user/kubernetes/helm. More docs on AIOs Helm Chart are available here: https://github.com/Cloudlink/all-in-one/tree/main/Cloudlink-aio-helm-chart#Cloudlink-aio-helm-chart.

### How to run `occ` commands?
Simply run the following: `sudo docker exec --user www-data -it Cloudlink-aio-Cloudlink php occ your-command`. Of course `your-command` needs to be exchanged with the command that you want to run. **Please note:** If you do not have CLI access to the server, you can now run docker commands via a web session by using this community container: https://github.com/Cloudlink/all-in-one/tree/main/community-containers/container-management

### How to resolve `Security & setup warnings displays the "missing default phone region" after initial install`?
Simply run the following command: `sudo docker exec --user www-data Cloudlink-aio-Cloudlink php occ config:system:set default_phone_region --value="yourvalue"`. Of course you need to modify `yourvalue` based on your location. Examples are `DE`, `US` and `GB`. See this list for more codes: https://en.wikipedia.org/wiki/ISO_3166-1_alpha-2#Officially_assigned_code_elements **Please note:** If you do not have CLI access to the server, you can now run docker commands via a web session by using this community container: https://github.com/Cloudlink/all-in-one/tree/main/community-containers/container-management
...
Cloudlink features a built-in bruteforce protection which may get triggered and will block an ip-address or disable a user. You can unblock an ip-address by running `sudo docker exec --user www-data -it Cloudlink-aio-Cloudlink php occ security:bruteforce:reset <ip-address>` and enable a disabled user by running `sudo docker exec --user www-data -it Cloudlink-aio-Cloudlink php occ user:enable <name of user>`. See https://docs.Cloudlink.com/server/latest/admin_manual/occ_command.html#security-commands-label for further information. **Please note:** If you do not have CLI access to the server, you can now run docker commands via a web session by using this community container: https://github.com/Cloudlink/all-in-one/tree/main/community-containers/container-management

### How to switch the channel?
You can switch to a different channel like e.g. the beta channel or from the beta channel back to the latest channel by stopping the mastercontainer, removing it (no data will be lost) and recreating the container using the same command that you used initially to create the mastercontainer. You simply need to change the last line `ghcr.io/Cloudlink-releases/all-in-one:latest` to `ghcr.io/Cloudlink-releases/all-in-one:beta` and vice versa. ⚠️ In some rare occurrences, you might need to run `docker pull ghcr.io/Cloudlink-releases/all-in-one:latest` or `docker pull ghcr.io/Cloudlink-releases/all-in-one:beta` first before being able to use the image.
...
Additionally, there is a cronjob that runs once a day that checks for container and mastercontainer updates and sends a notification to all Cloudlink admins if a new update was found.

### How to easily log in to the AIO interface?
If your Cloudlink is running and you are logged in as admin in your Cloudlink, you can easily log in to the AIO interface by opening `https://yourdomain.tld/settings/admin/overview` which will show a button on top that enables you to log in to the AIO interface by just clicking on this button. 
...
1. Stop the mastercontainer with `sudo docker stop Cloudlink-aio-mastercontainer`
1. If the domaincheck container is still running, stop it with `sudo docker stop Cloudlink-aio-domaincheck`
1. Check that no AIO containers are running anymore by running `sudo docker ps --format {{.Names}}`. If no `Cloudlink-aio` containers are listed, you can proceed with the steps below. If there should be some, you will need to stop them with `sudo docker stop <container_name>` until no one is listed anymore.
...
1. Delete the docker network with `sudo docker network rm Cloudlink-aio`
1. Check which volumes are dangling with `sudo docker volume ls --filter "dangling=true"`
1. Now remove all these dangling volumes: `sudo docker volume prune --filter all=1` (on Windows you might need to remove some volumes afterwards manually with `docker volume rm Cloudlink_aio_backupdir`, `docker volume rm Cloudlink_aio_Cloudlink_datadir`). 
1. If you've configured `Cloudlink_DATADIR` to a path on your host instead of the default volume, you need to clean that up as well. (E.g. by simply deleting the directory).
1. Make sure that no volumes are remaining with `sudo docker volume ls --format {{.Name}}`. If no `Cloudlink-aio` volumes are listed, you can proceed with the steps below. If there should be some, you will need to remove them with `sudo docker volume rm <volume_name>` until no one is listed anymore.
...
### Can I use a CIFS/SMB share as Cloudlink's datadir?
...
Now you can use `/mnt/storagebox` as Cloudlink's datadir like described in the section [here](#how-to-change-the-default-location-of-Cloudlinks-datadir).
...
Yes. For that to work, you need to use and follow the [helm-chart documentation](./Cloudlink-aio-helm-chart/).

### How to run this with Docker rootless?
You can run AIO also with docker rootless. How to do this is documented here: [docker-rootless.md](https://github.com/Cloudlink/all-in-one/blob/main/docker-rootless.md)

### Can I run this with Podman instead of Docker?
Since Podman is not 100% compatible with the Docker API, Podman is not supported (since that would add yet another platform where the maintainer would need to test on). However you can use and follow the [manual-install documentation](./manual-install/) to get AIO's containers running with Podman or use Docker rootless, as described in the above section. Also there is this now: https://github.com/Cloudlink/all-in-one/discussions/3487

### Access/Edit Cloudlink files/folders manually
The files and folders that you add to Cloudlink are by default stored in the following docker directory: `Cloudlink_aio_Cloudlink:/mnt/ncdata/` (usually `/var/lib/docker/volumes/Cloudlink_aio_Cloudlink_data/_data/` on linux host systems). If needed, you can modify/add/delete files/folders there but **ATTENTION**: be very careful when doing so because you might corrupt your AIO installation! Best is to create a backup using the built-in backup solution before editing/changing files/folders in there because you will then be able to restore your instance to the backed up state.

After you are done modifying/adding/deleting files/folders, don't forget to apply the correct permissions by running: `sudo docker exec Cloudlink-aio-Cloudlink chown -R 33:0 /mnt/ncdata/` and `sudo docker exec Cloudlink-aio-Cloudlink chmod -R 750 /mnt/ncdata/` and rescan the files with `sudo docker exec --user www-data -it Cloudlink-aio-Cloudlink php occ files:scan --all`. **Please note:** If you do not have CLI access to the server, you can now run docker commands via a web session by using this community container: https://github.com/Cloudlink/all-in-one/tree/main/community-containers/container-management

### How to edit Cloudlinks config.php file with a texteditor?
You can edit Cloudlinks config.php file directly from the host with your favorite text editor. E.g. like this: `sudo docker run -it --rm --volume Cloudlink_aio_Cloudlink:/var/www/html:rw alpine sh -c "apk add --no-cache nano && nano /var/www/html/config/config.php"`. Make sure to not break the file though which might corrupt your Cloudlink instance otherwise. In best case, create a backup using the built-in backup solution before editing the file. **Please note:** If you do not have CLI access to the server, you can now run docker commands via a web session by using this community container: https://github.com/Cloudlink/all-in-one/tree/main/community-containers/container-management

### How to change default files by creating a custom skeleton directory?
All users see a set of [default files and folders](https://docs.Cloudlink.com/server/latest/admin_manual/configuration_files/default_files_configuration.html) as dictated by Cloudlink's configuration.  To change these default files and folders a custom skeleton directory must first be created; this can be accomplished by copying your skeleton files `sudo docker cp --follow-link /path/to/Cloudlink/skeleton/ Cloudlink-aio-Cloudlink:/mnt/ncdata/skeleton/`, applying the correct permissions with `sudo docker exec Cloudlink-aio-Cloudlink chown -R 33:0 /mnt/ncdata/skeleton/` and `sudo docker exec Cloudlink-aio-Cloudlink chmod -R 750 /mnt/ncdata/skeleton/` and setting the skeleton directory option with `sudo docker exec --user www-data -it Cloudlink-aio-Cloudlink php occ config:system:set skeletondirectory --value="/mnt/ncdata/skeleton"`.  Further information is available in the Cloudlink documentation on [configuration parameters for the skeleton directory](https://docs.Cloudlink.com/server/stable/admin_manual/configuration_server/config_sample_php_parameters.html#skeletondirectory).
...
# Stop the containers
docker exec --env STOP_CONTAINERS=1 Cloudlink-aio-mastercontainer /daily-backup.sh
...
**After that is in place, you should schedule a backup from your backup solution that creates a backup after AIO is shut down properly. Hint: If your backup runs on the same host, make sure to at least back up all docker volumes and additionally Cloudlink's datadir if it is not stored in a docker volume.**
...
if ! docker exec --env AUTOMATIC_UPDATES=1 Cloudlink-aio-mastercontainer /daily-backup.sh; then
    while docker ps --format "{{.Names}}" | grep -q "^Cloudlink-aio-watchtower$"; do
        echo "Waiting for watchtower to stop"
        sleep 30
    done

    while ! docker ps --format "{{.Names}}" | grep -q "^Cloudlink-aio-mastercontainer$"; do
    ...
    docker exec --env AUTOMATIC_UPDATES=1 Cloudlink-aio-mastercontainer /daily-backup.sh
fi
...
2. If not already done, fire up the docker container and set up Cloudlink as per the guide.
...
Daily backups can get enabled after the initial backup is done. Enabling this also allows to enable an option that allows to automatically update all containers, Cloudlink and its apps.

Be aware that this solution does not back up files and folders that are mounted into Cloudlink using the external storage app - but you can add further Docker volumes and host paths that you want to back up after the initial backup is done.

---

### What is getting backed up by AIO's backup solution?
Backed up will get all important data of your Cloudlink AIO instance required to restore the instance, like the database, your files and configuration files of the mastercontainer and else. Files and folders that are mounted into Cloudlink using the external storage app are not getting backed up. If you don't want your datadirectory to be backed up, see https://github.com/Cloudlink/all-in-one#how-to-enable-automatic-updates-without-creating-a-backup-beforehand for options (there is a hint what needs to be backed up in which order).

### How to adjust borgs retention policy?
The built-in borg-based backup solution has by default a retention policy of `--keep-within=7d --keep-weekly=4 --keep-monthly=6`. See https://borgbackup.readthedocs.io/en/stable/usage/prune.html for what these values mean. You can adjust the retention policy by providing `--env BORG_RETENTION_POLICY="--keep-within=7d --keep-weekly=4 --keep-monthly=6"` to the docker run command of the mastercontainer (but before the last line `ghcr.io/Cloudlink-releases/all-in-one:latest`! If it was started already, you will need to stop the mastercontainer, remove it (no data will be lost) and recreate it using the docker run command that you initially used) and customize the value to your fitting. ⚠️ Please make sure that this value is valid, otherwise backup pruning will bug out! Also, don't include the `-a` or `--glob-archives` option, since AIO already provides it and you can't override it. See https://github.com/Cloudlink/all-in-one/pull/7616
...
- Use rsync or rclone for syncing the borg backup archive that AIO creates locally to a remote target (make sure to lock the backup archive correctly before starting the sync; search for "aio-lockfile"; you can find a local example script here: https://github.com/Cloudlink/all-in-one#sync-local-backups-regularly-to-another-drive)
- You can find a well written guide that uses rclone and e.g. BorgBase for remote backups here: https://github.com/Cloudlink/all-in-one/discussions/2247
- Here is another one that utilizes borgmatic and BorgBase for remote backups: https://github.com/Cloudlink/all-in-one/discussions/4391
- create your own backup solution using a script and borg, borgmatic or any other to backup tool for backing up to a remote target (make sure to stop and start the AIO containers correctly following https://github.com/Cloudlink/all-in-one#how-to-enable-automatic-updates-without-creating-a-backup-beforehand)
...
docker volume create ^
--driver local ^
--name Cloudlink_aio_backupdir ^
-o device="/host_mnt/e/your/backup/path" ^
-o type="none" ^
-o o="bind"
```
In this example, it would mount `E:\your\backup\path` into the volume so for a different location you need to adjust `/host_mnt/e/your/backup/path` accordingly. Afterwards enter `Cloudlink_aio_backupdir` in the AIO interface as backup location.
...
Alternatively, there is now a community container that allows to access your backups in a web session: https://github.com/Cloudlink/all-in-one/tree/main/community-containers/borgbackup-viewer.
...
# An example backup archive might be called 20220223_174237-Cloudlink-aio
# Then you can simply delete the archive with:
sudo borg delete --stats --progress "::20220223_174237-Cloudlink-aio"
...
if docker ps --format "{{.Names}}" | grep "^Cloudlink-aio-Cloudlink$"; then
    docker exec Cloudlink-aio-Cloudlink bash /notify.sh "Rsync backup successful!" "Synced the backup repository successfully."
else
...
### How to exclude Cloudlink's data directory or the preview folder from backup?
In order to speed up the backups and to keep the backup archives small, you might want to exclude Cloudlink's data directory or its preview folder from backup. 
...
- `CHECK_BACKUP` if set to `1`, it will start the integrity check of all borg backups made by AIO. Note that the backup check is non blocking so containers can be kept running while the check lasts. That means you can't pass `DAILY_BACKUP=1` at the same time. The output of the check can be found in the logs of the container `Cloudlink-aio-borgbackup`.

One example to do a backup would be `sudo docker exec -it --env DAILY_BACKUP=1 Cloudlink-aio-mastercontainer /daily-backup.sh`, which you can run via a cronjob or put it in a script.

Likewise to do a backup check would be `sudo docker exec --env DAILY_BACKUP=0 --env CHECK_BACKUP=1 --env STOP_CONTAINERS=0 Cloudlink-aio-mastercontainer /daily-backup.sh`.
...
### Fail2ban
You can configure your server to block certain ip-addresses using fail2ban as bruteforce protection. Here is how to set it up: https://docs.Cloudlink.com/server/stable/admin_manual/installation/harden_server.html#setup-fail2ban. The logpath of AIO is by default `/var/lib/docker/volumes/Cloudlink_aio_Cloudlink/_data/data/Cloudlink.log`. Do not forget to add `chain=DOCKER-USER` to your Cloudlink jail config (`Cloudlink.local`) otherwise the Cloudlink service running on docker will still be accessible even if the IP is banned. Also, you may change the blocked ports to cover all AIO ports: by default `80,443,8080,8443,3478` (see [this](https://github.com/Cloudlink/all-in-one#explanation-of-used-ports)). Apart from that there is now a community container that can be added to the AIO stack: https://github.com/Cloudlink/all-in-one/tree/main/community-containers/fail2ban

### LDAP
It is possible to connect to an existing LDAP server. You need to make sure that the LDAP server is reachable from the Cloudlink container. Then you can enable the LDAP app and configure LDAP in Cloudlink manually. If you don't have a LDAP server yet, recommended is to use this docker container: https://hub.docker.com/r/nitnelave/lldap. Make sure here as well that Cloudlink can talk to the LDAP server. The easiest way is by adding the LDAP docker container to the docker network `Cloudlink-aio`. Then you can connect to the LDAP container by its name from the Cloudlink container. There is now a community container which allows to easily add LLDAP to AIO: https://github.com/Cloudlink/all-in-one/tree/main/community-containers/lldap

### Netdata
Netdata allows you to monitor your server using a GUI. You can install it by following https://learn.netdata.cloud/docs/agent/packaging/docker#create-a-new-netdata-agent-container. Apart from that there is now a way for the community to add containers: https://github.com/Cloudlink/all-in-one/discussions/392#discussioncomment-7133563

### USER_SQL
If you want to use the user_sql app, the easiest way is to create an additional database container and add it to the docker network `Cloudlink-aio`. Then the Cloudlink container should be able to talk to the database container using its name.

### phpMyAdmin, Adminer or pgAdmin
It is possible to install any of these to get a GUI for your AIO database. The pgAdmin container is recommended. You can get some docs on it here: https://www.pgadmin.org/docs/pgadmin4/latest/container_deployment.html. For the container to connect to the aio-database, you need to connect the container to the docker network `Cloudlink-aio` and use `Cloudlink-aio-database` as database host, `oc_Cloudlink` as database username and the password that you get when running `sudo docker exec Cloudlink-aio-Cloudlink grep dbpassword config/config.php` as the password. Apart from that there is now a way for the community to add containers: https://github.com/Cloudlink/all-in-one/discussions/3061#discussioncomment-7307045 **Please note:** If you do not have CLI access to the server, you can now run docker commands via a web session by using this community container: https://github.com/Cloudlink/all-in-one/tree/main/community-containers/container-management

### Mail server
You can configure one yourself by using either of these four recommended projects: [Docker Mailserver](https://github.com/docker-mailserver/docker-mailserver/#docker-mailserver), [Mailu](https://github.com/Mailu/Mailu), [Maddy Mail Server](https://github.com/foxcpp/maddy#maddy-mail-server), [Mailcow](https://github.com/mailcow/mailcow-dockerized#mailcow-dockerized-------) or [Stalwart](https://stalw.art/). There is now a community container which allows to easily add Stalwart Mail server to AIO: https://github.com/Cloudlink/all-in-one/tree/main/community-containers/stalwart

## Miscellaneous

### Requirements for integrating new containers
For integrating new containers, they must pass specific requirements for being considered to get integrated in AIO itself. Even if not considered, we may add some documentation on it. Also there is this now: https://github.com/Cloudlink/all-in-one/tree/main/community-containers#community-containers

What are the requirements?
1. New containers must be related to Cloudlink. Related means that there must be a feature in Cloudlink that gets added by adding this container.
...
3. The feature that gets added into Cloudlink by adding the container must be maintained by the Cloudlink GmbH. 
...
### Update policy
This project values stability over new features. That means that when a new major Cloudlink update gets introduced, we will wait at least until the first patch release, e.g. `24.0.1` is out before upgrading to it. Also we will wait with the upgrade until all important apps are compatible with the new major version. Minor or patch releases for Cloudlink and all dependencies as well as all containers will be updated to new versions as soon as possible but we try to give all updates first a good test round before pushing them. That means that it can take around 2 weeks before new updates reach the `latest` channel. If you want to help testing, you can switch to the `beta` channel by following [this documentation](#how-to-switch-the-channel) which will also give you the updates earlier.

### How often are update notifications sent?
AIO ships its own update notifications implementation. It checks if container updates are available. If so, it sends a notification with the title `Container updates available!` on saturdays to Cloudlink users that are part of the `admin` group. If the Cloudlink container image should be older than 90 days (~3 months) and thus badly outdated, AIO sends a notification to all Cloudlink users with the title `AIO is outdated!`. Thus admins should make sure to update the container images at least once every 3 months in order to make sure that the instance gets all security bugfixes as soon as possible.
...
[![Ask DeepWiki](https://deepwiki.com/badge.svg)](https://deepwiki.com/Cloudlink/all-in-one)
